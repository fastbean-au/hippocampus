...
func (d *DB) searchOutboxDDL() string {
	...
	case driverMySQL:
		// The id collation matches the memories table's, so the document deleted is keyed exactly as
		// the memory that asked for it was.
		return `CREATE TABLE IF NOT EXISTS ` + searchOutboxTable + ` (
...
func (d *DB) initSearchOutbox() error {
	...
	//
	// MySQL has no CREATE INDEX IF NOT EXISTS, so it probes information_schema instead - the same
	// split the forgotten log's indexes use.
	name := "idx_" + searchOutboxTable + "_queued_at"
	columns := " (queued_at)"
	...
	if d.driver == driverMySQL {
		...
		return d.createMySQLIndexIfMissing(
			searchOutboxTable,
			name,
			`CREATE INDEX `+name+` ON `+searchOutboxTable+columns,
		)
	}

	if _, err := d.sql.Exec(`CREATE INDEX IF NOT EXISTS ` + name + ` ON ` + searchOutboxTable + columns); err != nil {
		log.Errorf("failed to index the search outbox: %s", err.Error())
