...
func TestStoreMethods_ErrorOnClosedDB(t *testing.T) {
	...
	checks := []struct {
		...
		{"MemoryIdsMatching", func() error { _, err := db.MemoryIdsMatching(ctx, MemoryFilter{Group: "a"}); return err }},
		{"EventIdsMatching", func() error { _, err := db.EventIdsMatching(ctx, EventFilter{Group: "a"}); return err }},
		{"EventIdsForMemories", func() error { _, err := db.EventIdsForMemories(ctx, []string{"m1"}); return err }},
		{"DeleteEvents", func() error { _, err := db.DeleteEvents(ctx, []string{"e1"}); return err }},
