...
func TestPostgres_EvictionDoesNotFlagEventWhenNothingDeleted(t *testing.T) {
	...
	evicted, err := database.EvictMemories(context.Background(), server, EvictionTarget{Bytes: 1 << 30})
	if err != nil {
		t.Fatalf("EvictMemories: %s", err)
	}

	if evicted.Memories != 0 || evicted.Events != 0 {
		t.Fatalf("expected nothing evicted (m1 recall-skipped), got %d memories / %d events", evicted.Memories, evicted.Events)
	}
