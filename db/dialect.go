...
type dialect struct {
	...
	// blobBytesFunc is the byte length of a blob column. Two of the three spell it LENGTH, which on
	// Postgres counts characters and has no meaning for BYTEA at all, so that one wants
	// OCTET_LENGTH. Used by the migration that backfills callback_queue.payload_bytes, which is the
	// only place a stored blob's size is read rather than written alongside it.
	blobBytesFunc string
