...
func resetCommandLine() {
	viper.Reset()
	pflag.CommandLine = pflag.NewFlagSet("kafka", pflag.ContinueOnError)
	pflag.CommandLine.SetOutput(os.NewFile(0, os.DevNull))
}

// cancelledCtx returns an already-cancelled context so kafka's consume loop returns immediately
// (its FetchMessage honours ctx), letting run's happy path be exercised without a live broker.
...
func TestServe_BadLogLevel(t *testing.T) {
	setupFlags(t, []string{"--log-level", "bogus", "--topic", "t", "--consumer-group", "g"})
...
func TestServe_RunsThrough(t *testing.T) {
	setupFlags(t, []string{"--log-level", "debug", "--topic", "t", "--consumer-group", "g"})
...
func TestRun_MissingTopic(t *testing.T) {
	setupFlags(t, []string{"--consumer-group", "g"})
...
func TestRun_MissingConsumerGroup(t *testing.T) {
	setupFlags(t, []string{"--topic", "t"})

	if err := run(context.Background()); err == nil {
		t.Errorf("run should error when --consumer-group is missing")
	}
}

func TestRun_HappyPathReturnsOnCancel(t *testing.T) {
	setupFlags(t, []string{"--topic", "t", "--consumer-group", "g"})
...
func TestRun_DialError(t *testing.T) {
	setupFlags(t, []string{"--topic", "t", "--consumer-group", "g", "--tls", "--tls-ca-cert", "/no/such/ca.pem"})
...
func TestRealMain_ServeErrorReturnsOne(t *testing.T) {
	...
	if code := realMain([]string{"--log-level", "bogus", "--topic", "t", "--consumer-group", "g"}); code != 1 {
