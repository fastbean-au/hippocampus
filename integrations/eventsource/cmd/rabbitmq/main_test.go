...
func resetCommandLine() {
	viper.Reset()
	pflag.CommandLine = pflag.NewFlagSet("rabbitmq", pflag.ContinueOnError)
...
func TestServe_BadLogLevel(t *testing.T) {
	setupFlags(t, []string{"--log-level", "bogus", "--queue", "q"})
...
func TestServe_RunsThrough(t *testing.T) {
	setupFlags(t, []string{"--log-level", "debug", "--queue", "q", "--amqp-url", "amqp://127.0.0.1:1/"})

	if err := serve(context.Background()); err == nil {
		t.Errorf("serve should surface run's dial error")
	}
}
...
func TestRun_MissingQueue(t *testing.T) {
	...
	if err := run(context.Background()); err == nil {
		t.Errorf("run should error when --queue is missing")
	}
}

func TestRun_ConnectFailsReturnsError(t *testing.T) {
	// A refused port makes amqp.Dial fail fast, exercising run's full happy path.
	setupFlags(t, []string{"--queue", "q", "--amqp-url", "amqp://127.0.0.1:1/"})

	if err := run(context.Background()); err == nil {
		t.Errorf("run should surface the RabbitMQ dial error")
	}
}

func TestRun_DialError(t *testing.T) {
	setupFlags(t, []string{"--queue", "q", "--tls", "--tls-ca-cert", "/no/such/ca.pem"})
