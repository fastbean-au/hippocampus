...
- **[Group Scoping](docs/configuration.md#group-scoping):** A tier says what a caller may do; a `groups` claim says which records they may do it to. Scoped tokens see, write, search and export only their own group labels, with out-of-scope records reporting `NotFound` rather than confirming they exist. Off until tokens carry the claim, and a **soft** partition rather than isolation — the decay dynamics stay store-global, so read [the trust boundary](docs/operations.md#group-scoping-and-the-trust-boundary) before relying on it; hard isolation is one instance per tenant.
