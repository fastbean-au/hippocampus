...
// tierContextKey is the unexported key type under which the caller's resolved authorization tier is
// stashed. Like claimsContextKey, an unexported type prevents collisions from other packages.
type tierContextKey struct{}

// ContextWithTier returns a copy of ctx carrying the caller's resolved authorization tier. Both
// authorization adapters (the gRPC interceptor and the gateway middleware) call it once they have
...
// TierFromContext returns the resolved authorization tier stashed by ContextWithTier. The bool is
// false when no tier is present - authorization was not run (auth disabled), so callers should
