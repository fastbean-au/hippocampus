...
| `update_memory`                | `UpdateMemory`               | Revise a memory by id; only fields you set change, significance `0` leaves it unchanged. `metadata` **replaces** the stored labels wholesale; `clear_metadata`/`clear_group` remove them, since an omitted field means "leave unchanged".    |
...
| `search_memories`              | `SearchMemories`             | Content search (built in on `sqlite`; needs OpenSearch on `postgres`/`mysql`). `mode` selects keyword (default), semantic, or hybrid; the latter two need the service to have an embedding model and OpenSearch. `reinforce` off by default. |
...
| `list_memories`                | `GetMemories`                | Read-only browse by group, significance, and `metadata` labels; `recalled: false` finds memories never recalled. Does **not** reinforce.                                                                                                     |
| `create_event`                 | `StoreEvent`                 | A named time span memories can be grouped under; takes `metadata` labels like a memory.                                                                                                                                                      |
| `list_events`                  | `GetEvents`                  | Read-only browse of events, filterable by `metadata` labels.                                                                                                                                                                                 |
