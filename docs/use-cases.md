...
forgets the rest, most-worthless-first, to stay within a capacity bound.
...
- **Operational / audit event history** — keep every deploy, incident, and config change while they
  ...
  investigation touches so they survive. `group` labels scope events to a system or team.
...
lose: forgetting is the point, and the service has no visibility into memory *content* (bodies are
...
Ideal where each producer keeps its *own* bounded memory — one instance per device or per process.
...
Because decay, capacity pressure, and eviction are **global** dynamics over a store, tenancy is *not*
